...
function isValidInfo() {
    ...
    var CCCDId = document.getElementById("cccd-id").value;
    var AddressId = document.getElementById("address-id").value;

    return name && dob && CCCDId && AddressId
}
...
function submitQuizz() {
    ...
    const studentInfo = {
        ...
        CCCDId: document.getElementById("cccd-id").value,
    ...
    let html = `
        <div class="result">
            ...
            <p class="my-2">Căn cước công dân: ${result.studentInfo.CCCDId}</p>
            <p class="my-2">Địa chỉ: ${result.studentInfo.AddressId}</p>
